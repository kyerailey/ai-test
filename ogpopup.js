...
document.getElementById('scrapeBtn').addEventListener('click', async () => {
  ...
  chrome.scripting.executeScript({
    ...
    // UI UPDATE: Change status message to detected concentration
    ...
    // Sort: Not Satisfied > In Progress > Satisfied
    ...
    const renderItem = (item) => {
      ...
      // Don't deep audit general elective buckets
      ...
      if (jsonReq && !isGenericElective) {
        jsonReq.courses.forEach(courseObj => {
          ...
          if (baseCode.endsWith("+")) return; // Skip wildcard patterns like "CSC 2000+"
